...
``` r
select(litters_df, litter_number, everything())  # litter_number变第一列
```
...
``` r
filter(litters_df, gd_of_birth == 20)  # == is to test the equality
```
...
``` r
filter(litters_df, group %in% c("Con7", "Mod8"))  # both Con7 Mod8
```
...
``` r
arrange(litters_df, pups_born_alive) # arrange more than one?
```
